...
load_census_data <- function(census_data = "../data-cleaned/2010-census.tsv") {
  ...
  if (file.exists(census_data)) {
    ...
    get_decennial(
      geography = "state", 
      variables = "P001001",
      year = 2010 # I Had to pick a year for the 2000s NEH grant data, so I settled on 2010.
...
grants_with_population_data |> 
  ...
  summarize(total_awarded_to_state = (sum(grant_total))) |> 
...
While we only excluded `percentage_of_grants_without_population_data` percent of the data, that still excludes entire US regions and territories.
...
total_awarded_to_all_us_territories <- grants_2000s |> 
  mutate(total_awarded = AwardOutright + AwardMatching) |> 
...
In the 2000s the NEH awarded `r total_awarded_to_all_us_territories` dollars, of which `r total_awarded_to_non_us_territories` was excluded from this analysis. That exclusion represents about `r percent_awards_excluded`
