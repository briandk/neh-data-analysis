...
In this brief report, we examine how the total NEH award dollars and matching funds are distributed on a per-state, per-person basis.
Breaking the data down per person helps correct for the population effects of very low-population and very high-population states because states are considered proportionately, rather than by their strict population.
There are three major findings:

1.  Most states hover around the national average of `r scales::dollar(3.52)` US dollars total awarded per person.
2.  Washington, DC has the highest dollars per person awarded at roughly `r scales::dollar(45)`. That's 3 times the next highest state.
3. In terms of the percentage share of total dollars awarded, New York leads at around 12\%, with California behind it at roughly 6\%. This seems disproportionate given their per capita dollars awarded.
...
At around `r scales::dollar(45)` DC has more NEH dollars awarded per-person number than any other state in the US.
...
Extremely low outliers are Florida and Texas at roughly 2 dollars per person, while at the upper end Washington DC has roughly US dollars per person, about 3 times the amount of grant dollars per person than the next highest state of Vermont.
