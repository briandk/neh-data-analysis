...
# Research Question - What's the Per-Capita Awarded for Each State?
...
awards_per_capita_by_state |> 
  ...
  geom_point(
    aes(x = forcats::fct_reorder(census_state, per_capita_award), 
        y = per_capita_award,
        size = population,
        color = total_awarded_to_state),
    ) + 
  coord_flip() +
  geom_hline(
    yintercept = national_mean_per_capita_award,
    color = "darkblue",
    alpha = 0.5,
  ) +
...
Because of our exclusion, we can't say with certainty what the NEH award dollars per capita in those regions represents.
